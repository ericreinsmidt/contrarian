 ...
 * Modification: keep the front F1/F2 keys out of the core -- see below.
 */
...
void input_poll_callback(void) {
	...
	for (int i=0; config.controls[i].name; i++) {
		...
		/* Contrarian: the Brick's front F1/F2 keys report on the gamepad node
		 * as BTN_THUMBL/BTN_THUMBR, which is what this mapping calls L3/R3.
		 * Contrarian owns them for brightness, so every brightness press was
		 * also being handed to the core as an L3/R3 input -- the "smearing".
		 * The NES has no stick clicks, so nothing is lost by dropping them. */
		if (mapping->retro==RETRO_DEVICE_ID_JOYPAD_L3 ||
		    mapping->retro==RETRO_DEVICE_ID_JOYPAD_R3) continue;
		int btn = 1 << mapping->local;
