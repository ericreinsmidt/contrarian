 ...
 * Modification: the in-game menu is the console's own front panel.
 *
 * Contrarian has no settings, so a list of settings is the wrong object. The
 * menu is a photograph of an NES front panel with two working buttons:
 * POWER quits back to the launcher, RESET restarts the game. Nothing else,
 * because there is nothing else.
 *
 * Implemented as a self-contained loop inserted before minarch's own. It sets
 * show_menu to 0 on its way out, so the original loop below is skipped and
 * the existing teardown still runs untouched.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---- Contrarian: NES front panel ---------------------------------------- */

#define CTR_BTN_POWER 0
#define CTR_BTN_RESET 1

/* Measured from res/nes/geometry.json, in the panel image's own pixels. */
#define CTR_PANEL_W 800
#define CTR_PANEL_H 276
static const SDL_Rect CTR_BTN[2] = {
	{ 151, 180, 71, 37 },   /* POWER */
	{ 230, 180, 71, 37 },   /* RESET */
};

static SDL_Surface* ctr_panel_img;   /* LED dark */
static SDL_Surface* ctr_panel_lit;   /* LED lit  */
static int          ctr_panel_tried;

static SDL_Surface* ctr_load_panel(const char* dir, const char* name)
{
	char path[512];
	SDL_Surface* raw;
	SDL_Surface* conv;
...
static void ctr_panel_init(void)
{
	const char* dir = getenv("CONTRARIAN_PANEL");
	if (ctr_panel_tried) return;
	ctr_panel_tried = 1;
	if (!dir || !*dir) return;
	ctr_panel_img = ctr_load_panel(dir, "panel.png");
	ctr_panel_lit = ctr_load_panel(dir, "panel_lit.png");
	if (ctr_panel_img)
		LOG_info("contrarian: front panel loaded from %s\n", dir);
}

/* A hollow rectangle, drawn as four fills -- the selected button gets an
 * outline rather than a tint, so the button face stays readable. */
static void ctr_outline(SDL_Surface* dst, SDL_Rect r, int t, Uint32 c)
{
	SDL_FillRect(dst, &(SDL_Rect){ r.x - t,     r.y - t,     r.w + t*2, t }, c);
	SDL_FillRect(dst, &(SDL_Rect){ r.x - t,     r.y + r.h,   r.w + t*2, t }, c);
	SDL_FillRect(dst, &(SDL_Rect){ r.x - t,     r.y,         t,         r.h }, c);
	SDL_FillRect(dst, &(SDL_Rect){ r.x + r.w,   r.y,         t,         r.h }, c);
}

/* Returns 1 if it handled the menu. Leaves show_menu at 0. */
static int ctr_panel_loop(void)
{
	SDL_Rect panel;
	float scale;
	int selected = CTR_BTN_RESET;   /* the safer of the two starts under the cursor */
	int dirty = 1;

	ctr_panel_init();
	if (!ctr_panel_img) return 0;

	scale  = (float)(DEVICE_WIDTH * 0.90f) / (float)CTR_PANEL_W;
	panel.w = (int)(CTR_PANEL_W * scale);
	panel.h = (int)(CTR_PANEL_H * scale);
	panel.x = (DEVICE_WIDTH  - panel.w) / 2;
	panel.y = (DEVICE_HEIGHT - panel.h) / 2;

	while (show_menu) {
		GFX_startFrame();
		PAD_poll();

		/* Left/right only. The buttons sit side by side on the console, so
		 * up and down have nothing to mean here. */
		if (PAD_justPressed(BTN_LEFT)) {
			selected = CTR_BTN_POWER; dirty = 1;
		}
		else if (PAD_justPressed(BTN_RIGHT)) {
			selected = CTR_BTN_RESET; dirty = 1;
		}
		else if (PAD_justPressed(BTN_B) || PAD_justPressed(BTN_MENU)) {
			show_menu = 0;                       /* back to the game */
		}
		else if (PAD_justPressed(BTN_A)) {
			/* 'status' is a local of Menu_loop and only drives a post-menu
			 * notification, so it is deliberately not set from here. */
			if (selected == CTR_BTN_POWER) {
				show_menu = 0;
				quit = 1;
			} else {
				core.reset();
		...
		if (dirty && show_menu) {
			...
			GFX_clear(screen);
			/* the paused game, dimmed, behind the console */
			GFX_drawOnLayer(menu.bitmap, 0, 0, DEVICE_WIDTH, DEVICE_HEIGHT, 0.25f, 1, 0);

			/* The set is on while you are in here, so the LED is lit -- until
			 * POWER is the button under the cursor, when it goes dark to say
			 * what pressing it will do. */
			img = (selected == CTR_BTN_POWER || !ctr_panel_lit)
			    ? ctr_panel_img : ctr_panel_lit;
			SDL_BlitScaled(img, NULL, screen, &panel);

			hi = SDL_MapRGB(screen->format, 245, 232, 200);
			ctr_outline(screen, (SDL_Rect){
				panel.x + (int)(CTR_BTN[selected].x * scale),
				panel.y + (int)(CTR_BTN[selected].y * scale),
				(int)(CTR_BTN[selected].w * scale),
				(int)(CTR_BTN[selected].h * scale),
			}, 3, hi);
...
void Menu_loop(void) {
	...
	/* Contrarian: the front panel handles the menu and leaves show_menu at 0,
	 * so the loop below is skipped and the teardown after it still runs. */
	ctr_panel_loop();
