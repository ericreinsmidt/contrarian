 ...
 * Modification: the settings indicator is the launcher's thin top line
 * instead of NextUI's corner pill, and it holds as long as the launcher's.
 */
...
/* The shared indicator, drawn identically here and in the launcher.
 * Defined in the common/api.c override. */
extern void ctr_draw_setting_line(SDL_Surface *dst, int kind);
...
void Notification_update(uint32_t now) {
    ...
    if (system_indicator_type != SYSTEM_INDICATOR_NONE) {
        ...
        /* Contrarian: hold as long as the launcher does (OSD_WINDOW_MS). */
        if (elapsed >= 900u) {
...
static void render_system_indicator(void) {
    /* Contrarian: the thin top line, drawn straight into the full-screen
     * notification layer, instead of NextUI's top-right pill. Colortemp is
     * ignored -- Contrarian never changes it, so it has nothing to report. */
    ...
    if (kind) ctr_draw_setting_line(gl_notification_surface, kind);
}
