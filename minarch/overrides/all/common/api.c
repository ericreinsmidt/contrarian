 ...
 * Overriding this file is the only way to do it. GFX_init() calls
 * LEDS_initLeds() from inside api.c itself, so removing the call in
 * minarch.c changes nothing -- the lights come up with NextUI's defaults
 * (white, brightness 100) and LEDS_applyRules() puts them back every frame.
 *
 * Kill the led_anim engine and every raw channel directly, rather than
 * setting an "off" profile, so it holds no matter what state the animation
 * engine was left in by the stock input daemon. */
static void ctr_leds_off(void)
{
	int fd;
	char path[96];
...
{
	/* Contrarian: never light them, battery indicators included. */
	(void)indicator_only;
	ctr_leds_off();
	return;
...
void LEDS_initLeds()
{
	...
	/* Contrarian: kill them instead of applying a profile. */
	ctr_leds_off();
}
